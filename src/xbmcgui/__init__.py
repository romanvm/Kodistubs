...
Offers classes and functions that manipulate the Graphical User Interface through windows, dialogs, and various control
widgets.
"""
...
ACTION_DIALOG_SELECT_AUDIO = 271
ACTION_DIALOG_SELECT_SUBTITLE = 272
ACTION_DIALOG_SELECT_VIDEO = 270
ACTION_ENTER = 135
ACTION_ERROR = 998
ACTION_FILTER = 233
...
ACTION_KEYBOARD_COMPOSING_KEY = 263
ACTION_KEYBOARD_COMPOSING_KEY_CANCELLED = 264
ACTION_KEYBOARD_COMPOSING_KEY_FINISHED = 265
...
ACTION_PLAYER_DECREASE_TEMPO = 267
ACTION_PLAYER_FORWARD = 77
ACTION_PLAYER_INCREASE_TEMPO = 266
...
class ListItem:
    ...
    **Selectable window list item.**
    ...
    def setInfo(self, type: str, infoLabels: Dict[str, Union[str, List[Union[str, Tuple[str, str]]]]]) -> None:
        """
        ...
        **Available types**
        ...
        .. note::
            ...
            To set pictures exif info, prepend ``exif:`` to the label. Exif values must be passed as strings, separate
            value pairs with a comma. **(eg.** **``{'exif:resolution': '720,480'}``** See kodi_pictures_infotag for
            valid strings. You can use the above as keywords for arguments and skip certain optional arguments. Once you
            use a keyword, all following arguments require the keyword.
        ...
        **General Values** (that apply to all types):
        ...
        +---------------+--------------------------------------------------------------------------------------------+
        | cast          | list (["Michal C. Hall","Jennifer Carpenter"]) - if provided a list of tuples cast will be |
        |               | interpreted as castandrole                                                                 |
        +---------------+--------------------------------------------------------------------------------------------+
        | castandrole   | list of tuples ([("Michael C. Hall","Dexter"),("Jennifer Carpenter","Debra")])             |
        +---------------+--------------------------------------------------------------------------------------------+
        | director      | string (Dagur Kari) or list of strings (["Dagur Kari", "Quentin Tarantino", "Chrstopher    |
        |               | Nolan"])                                                                                   |
        +---------------+--------------------------------------------------------------------------------------------+
        | mpaa          | string (PG-13)                                                                             |
        +---------------+--------------------------------------------------------------------------------------------+
        ...
        +---------------+--------------------------------------------------------------------------------------------+
        | writer        | string (Robert D. Siegel) or list of strings (["Robert D. Siegel", "Jonathan Nolan", "J.K. |
        |               | Rowling"])                                                                                 |
        +---------------+--------------------------------------------------------------------------------------------+
        ...
        +---------------+--------------------------------------------------------------------------------------------+
        | credits       | string (Andy Kaufman) or list of strings (["Dagur Kari", "Quentin Tarantino", "Chrstopher  |
        |               | Nolan"]) - writing credits                                                                 |
        +---------------+--------------------------------------------------------------------------------------------+
        ...
        +---------------+--------------------------------------------------------------------------------------------+
        | dbid          | integer (23) - Only add this for items which are part of the local db. You also need to    |
        |               | set the correct 'mediatype'!                                                               |
        +---------------+--------------------------------------------------------------------------------------------+
        ...
        +--------------------------+---------------------------------------------------------------------------------+
        | dbid                     | integer (23) - Only add this for items which are part of the local db. You also |
        |                          | need to set the correct 'mediatype'!                                            |
        +--------------------------+---------------------------------------------------------------------------------+
        ...
        @python_v17 Added labels **setid**, **set**, **imdbnumber**, **code**, **dbid** (video), **path** and
        **userrating**. Expanded the possible infoLabels for the option **mediatype**.

        @python_v18 Added new **game** type and associated infolabels. Added labels **dbid** (music), **setoverview**,
        **tag**, **sortepisode**, **sortseason**, **episodeguide**, **showlink**. Extended labels **genre**,
        **country**, **director**, **studio**, **writer**, **tag**, **credits** to also use a list of strings.

        @python_v20 Partially deprecated. Use explicit setters in **InfoTagVideo**, **InfoTagMusic**, **InfoTagPicture**
        or **InfoTagGame** instead.
    ...
    def setCast(self, actors: List[Dict[str, str]]) -> None:
        ...
        - Keys:
          ...
          role      string (Dexter)                                                                        
          thumbnail string (`http://www.someurl.com/someimage.png <http://www.someurl.com/someimage.png>`_)
          order     integer (1)                                                                            
          ========= =======================================================================================
    ...
    def addStreamInfo(self, cType: str, dictionary: Dict[str, str]) -> None:
        ...
        @python_v20 Deprecated. Use **InfoTagVideo.addVideoStream()**, **InfoTagVideo.addAudioStream()** or
        **InfoTagVideo.addSubtitleStream()** instead.
    ...
    def addContextMenuItems(self, items: List[Tuple[str, str]], replaceItems: bool = False) -> None:
        """
    ...
    def setProperty(self, key: str, value: str) -> None:
        ...
        .. note::
            ...
            Key is NOT case sensitive. You can use the above as keywords for arguments and skip certain optional
            arguments. Once you use a keyword, all following arguments require the keyword. Some of these are treated
            internally by Kodi, such as the 'StartOffset' property, which is the offset in seconds at which to start
            playback of an item. Others may be used in the skin to add extra information, such as 'WatchedCount' for
            tvshow items
        ...
        - **Internal Properties**
          ...
          | Key                | Description                                                                           |
          +====================+=======================================================================================+
          | inputstream        | string (inputstream.adaptive) - Set the inputstream add-on that will be used to play  |
          |                    | the item                                                                              |
          +--------------------+---------------------------------------------------------------------------------------+
          | IsPlayable         | string - "true", "false" - Mark the item as playable, **mandatory for playable        |
          |                    | items**                                                                               |
          +--------------------+---------------------------------------------------------------------------------------+
          ...
          +--------------------+---------------------------------------------------------------------------------------+
          | SpecialSort        | string - "top", "bottom" - The item will remain at the top or bottom of the current   |
          |                    | list                                                                                  |
          +--------------------+---------------------------------------------------------------------------------------+
          ...
          +--------------------+---------------------------------------------------------------------------------------+
          | ForceResolvePlugin | string - "true", "false" - When true ensures that a plugin will always receive        |
          |                    | callbacks to resolve paths (useful for playlist cases)                                |
          +--------------------+---------------------------------------------------------------------------------------+
          | rtsp_transport     | string - "udp", "udp_multicast" or "tcp" - Allow to force the rtsp transport mode for |
          |                    | rtsp streams                                                                          |
          +--------------------+---------------------------------------------------------------------------------------+
    ...
    def getProperty(self, key: str) -> str:
        ...
        @python_v20 **ResumeTime** and **TotalTime** deprecated. Use **InfoTagVideo.getResumeTime()** and
        **InfoTagVideo.getResumeTimeTotal()** instead.
...
class Control:
    ...
    **Code based skin access.**
    ...
    Kodi is noted as having a very flexible and robust framework for its GUI, making theme-skinning and personal
    customization very accessible. Users can create their own skin (or modify an existing skin) and share it with
    others.

    Kodi includes a new GUI library written from scratch. This library allows you to skin/change everything you see in
    Kodi, from the images, the sizes and positions of all controls, colours, fonts, and text, through to altering
    navigation and even adding new functionality. The skin system is quite complex, and this portion of the manual is
    dedicated to providing in depth information on how it all works, along with tips to make the experience a little
    more pleasant.
    """

    def canAcceptMessages(self, actionId: int) -> bool:
        return True
    ...
    def setVisible(self, visible: bool) -> None:
        ...
        @python_v19 You can now define the visible state of a control before it being added to a window. This value will
        be taken into account when the control is later added.
    ...
    def isVisible(self) -> bool:
        ...
        .. note::
            ...
            If a given control is set visible (c.f. setVisible() but was not yet added to a window, this method will
            return ``False`` (the control is not visible yet since it was not added to the window).
    ...
    def setVisibleCondition(self, visible: str, allowHiddenFocus: bool = False) -> None:
        """
        Sets the control's visible condition.

        Allows Kodi to control the visible status of the control.

        `List of Conditions <http://kodi.wiki/view/List_of_Boolean_Conditions>`_
    ...
    def setAnimations(self, eventAttr: List[Tuple[str, str]]) -> None:
        ...
        **[(event,attr,)\\*]**: list - A list of tuples consisting of event and attributes pairs.
        ...
        `Animating your skin <http://kodi.wiki/view/Animating_Your_Skin>`_
    ...
    def setNavigation(self, up: Control, down: Control, left: Control, right: Control) -> None:
        """
        ...
        .. note::
            ...
            Same as controlUp(), controlDown(), controlLeft(), controlRight(). Set to self to disable navigation for
            that direction.
...
class ControlSpin(Control):
    ...
    **Code based skin access.**
    ...
    The spin control is used for when a list of options can be chosen (such as a page up/down control). You can choose
    the position, size, and look of the spin control.
    ...
    .. warning::
        ...
        **Not working yet**. You can't create this object, it is returned by objects like ControlTextBox and
        ControlList.
    """

    def setTextures(self, up: str, down: str, upFocus: str, downFocus: str, upDisabled: str, downDisabled: str) -> None:
        """
...
class ControlLabel(Control):
    ...
    **Used to show some lines of text.**
    ...
    The label control is used for displaying text in Kodi. You can choose the font, size, colour, location and contents
...
class ControlEdit(Control):
    ...
    **Used as an input control for the osd keyboard and other input fields.**
    ...
    The edit control allows a user to input text in Kodi. You can choose the font, size, colour, location and header of
    ...
    .. note::
        ...
        following arguments require the keyword. After you create the control, you need to add it to the window with
        addControl().
    ...
    def setType(self, type: int, heading: str) -> None:
        ...
        :param heading: string or unicode - heading that will be used for to numeric or keyboard dialog when the edit
            control is clicked.
...
class ControlList(Control):
    ...
    **Used for a scrolling lists of items.** **Replaces the list control.**
    ...
    The list container is one of several containers used to display items from file lists in various ways. The list
    container is very flexible - it's only restriction is that it is a list - i.e. a single column or row of items. The
    ...
    def __init__(
        ...
        _itemHeight: int = 27,
        _space: int = 2,
        _alignmentY: Optional[int] = None,
    ) -> None:
        pass

    def addItem(self, item: Union[str, ListItem], sendMessage: bool = True) -> None:
        """
    ...
    def reset(self) -> None:
        ...
        **Examples:**
        ...
        destruction. The example assumes ``self`` as a ``WindowXMLDialog`` instance containing a ``ControlList`` with id
        = 800. The class preserves the ``ListItem`` objects in a class member variable.

        .. warning::

            Calling ``reset()`` will destroy any ``ListItem`` objects in the ``ControlList`` if not hold by any other
            class. Make sure you you don't call ``addItems()`` with the previous ``ListItem`` references after calling
            ``reset()``. If you need to preserve the ``ListItem`` objects after ``reset()`` make sure you store them as
            members of your ``WindowXML`` class (see examples).
    ...
    def getSpinControl(self) -> Control:
        ...
        .. warning::
            ...
            Not working completely yet After adding this control list to a window it is not possible to change the
            settings of this spin control.
    ...
    def getSelectedItem(self) -> ListItem:
        ...
        .. note::
            ...
            empty lists. See windowexample.py on how to use this.
...
class ControlFadeLabel(Control):
    ...
    **Used to show multiple pieces of text in the same position, by fading from one to the other.**
    ...
    The fade label control is used for displaying multiple pieces of text in the same space in Kodi. You can choose the
    font, size, colour, location and contents of the text to be displayed. The first piece of information to display
    fades in over 50 frames, then scrolls off to the left. Once it is finished scrolling off screen, the second piece of
    information fades in and the process repeats. A fade label control is not supported in a list container.
...
class ControlTextBox(Control):
    ...
    **Used to show a multi-page piece of text.**
    ...
    The text box is used for showing a large multipage piece of text in Kodi. You can choose the position, size, and
    look of the text.

    As stated above, the GUI control is only created once added to a window. The example below shows how a
    ControlTextBox can be created, added to the current window and have some of its properties changed.
    ...
    **Extended example:**
    ...
    def setText(self, text: str) -> None:
        ...
        @python_v19 setText can now be used before adding the control to the window (the defined value is taken into
        consideration when the control is created)
...
class ControlImage(Control):
    ...
    **Used to show an image.**
    ...
    The image control is used for displaying images in Kodi. You can choose the position, size, transparency and
    contents of the image to be displayed.
    ...
    :param aspectRatio: [opt] integer - (values 0 = stretch (default), 1 = scale up (crops), 2 = scale down (black bar)
    ...
    def __init__(
        ...
        filename: str,
        aspectRatio: int = 0,
        colorDiffuse: Optional[str] = None,
    ) -> None:
        pass
...
class ControlProgress(Control):
    ...
    **Used to show the progress of a particular operation.**
    ...
    The progress control is used to show the progress of an item that may take a long time, or to show how far through a
    movie you are. You can choose the position, size, and look of the progress control.
    ...
    :param height: integer - height of control.
    :param filename: string - image filename.
    :param texturebg: [opt] string - specifies the image file whichshould be displayed in the background of the progress
        control.
    :param textureleft: [opt] string - specifies the image file whichshould be displayed for the left side of the
        progress bar. This is rendered on the left side of the bar.
    :param texturemid: [opt] string - specifies the image file which should be displayed for the middl portion of the
        progress bar. This is the ``fill`` texture used to fill up the bar. It's positioned on the right of the
        ``<lefttexture>`` texture, and fills the gap between the ``<lefttexture>`` and ``<righttexture>`` textures,
        depending on how far progressed the item is.
    :param textureright: [opt] string - specifies the image file which should be displayed for the right side of the
        progress bar. This is rendered on the right side of the bar.
    :param textureoverlay: [opt] string - specifies the image file which should be displayed over the top of all other
        images in the progress bar. It is centered vertically and horizontally within the space taken up by the
        background image.
    ...
    Example::
        ...
        ...
        # ControlProgress(x, y, width, height, filename[, texturebg, textureleft, texturemid, textureright,
        # textureoverlay])
    ...
    def __init__(
        ...
        textureoverlay: Optional[str] = None,
    ) -> None:
        pass
...
class ControlButton(Control):
    ...
    **A standard push button control.**
    ...
    The button control is used for creating push buttons in Kodi. You can choose the position, size, and look of the
    button, as well as choosing what action(s) should be performed when pushed.
...
class ControlGroup(Control):
    ...
    **Used to group controls together..**
    ...
    The group control is one of the most important controls. It allows you to group controls together, applying
    attributes to all of them at once. It also remembers the last navigated button in the group, so you can set the
    **``<onup>``** of a control to a group of controls to have it always go back to the one you were at before. It also
    allows you to position controls more accurately relative to each other, as any controls within a group take their
    coordinates from the group's top left corner (or from elsewhere if you use the **"r"** attribute). You can have as
    many groups as you like within the skin, and groups within groups are handled with no issues.
...
class ControlRadioButton(Control):
    ...
    **A radio button control (as used for on/off settings).**
    ...
    The radio button control is used for creating push button on/off settings in Kodi. You can choose the position,
    size, and look of the button, as well as the focused and unfocused radio textures. Used for settings controls.
    ...
    :param alignment: [opt] integer - alignment of label
    :param font: [opt] string - font used for label text. (e.g. 'font13')
    :param textColor: [opt] hexstring - color of label when control is enabled. radiobutton's label. (e.g. '0xFFFFFFFF')
    :param disabledColor: [opt] hexstring - color of label when control is disabled. (e.g. '0xFFFF3300')
    ...
    def __init__(
        ...
        disabledOffTexture: Optional[str] = None,
    ) -> None:
        pass
    ...
    def setLabel(
        ...
        :param label: string or unicode - text string.
        :param font: [opt] string - font used for label text. (e.g. 'font13')
        :param textColor: [opt] hexstring - color of enabled radio button's label. (e.g. '0xFFFFFFFF')
        :param disabledColor: [opt] hexstring - color of disabled radio button's label. (e.g. '0xFFFF3300')
        :param shadowColor: [opt] hexstring - color of radio button's label's shadow. (e.g. '0xFF000000')
        :param focusedColor: [opt] hexstring - color of focused radio button's label. (e.g. '0xFFFFFF00')
    ...
    def setRadioDimension(self, x: int, y: int, width: int, height: int) -> None:
        """
...
class ControlSlider(Control):
    ...
    **Used for a volume slider.**
    ...
    The slider control is used for things where a sliding bar best represents the operation at hand (such as a volume
    control or seek control). You can choose the position, size, and look of the slider control.
    ...
    def __init__(
        ...
        texturedisabled: Optional[str] = None,
    ) -> None:
        pass
    ...
    def setFloat(self, value: float, min: float, delta: float, max: float) -> None:
        """
        Sets the range, value and step size of the slider.
...
class Dialog:
    ...
    **Kodi's dialog class**
    ...
    The graphical control element dialog box (also called dialogue box or just dialog) is a small window that
    ...
    def yesno(
        ...
        :param defaultbutton: [opt] integer - specifies the default focused button. *(default=DLG_YESNO_NO_BTN)*
    ...
    def yesnocustom(
        ...
        """
        The YesNoCustom dialog can be used to inform the user about questions and get the answer. The dialog provides a
        third button apart from yes and no. Button labels are fully customizable.
    ...
    def multiselect(
        ...
        :param autoclose: [opt] integer - milliseconds to autoclose dialog. (default=do not autoclose)
        :param preselect: [opt] list of int - indexes of items to preselect in list (default: do not preselect any item)
        :param useDetails: [opt] bool - use detailed list instead of a compact list. (default=false)
    ...
    def browse(
        ...
        :param heading: string or unicode - dialog heading.
        :param shares: string or unicode - from `sources.xml <http://kodi.wiki/view/Sources.xml>`_
        ...
        :return: If enableMultiple is False (default): returns filename and/or path as a string to the location of the
            highlighted item, if user pressed 'Ok' or a masked item was selected. Returns the default value if dialog
            was canceled. If enableMultiple is True: returns tuple of marked filenames as a string if user pressed 'Ok'
            or a masked item was selected. Returns empty tuple if dialog was canceled. If type is 0 or 3 the
            enableMultiple parameter is ignore
    ...
    def browseSingle(
        ...
        :return: Returns filename and/or path as a string to the location of the highlighted item, if user pressed 'Ok'
            or a masked item was selected. Returns the default value if dialog was canceled.
    ...
    def notification(self, heading: str, message: str, icon: str = "", time: int = 0, sound: bool = True) -> None:
        """
    ...
    def colorpicker(
        ...
        selectedcolor: str = "",
        colorfile: str = "",
        colorlist: Optional[List[ListItem]] = None,
    ) -> str:
        """
        :param heading: string - dialog heading.
        ...
        :param colorlist: [opt] xbmcgui.ListItems - where label defines the color name and label2 is set to the hex
            value.
        ...
        Example::
            ...
            # colorfile example
            dialog = xbmcgui.Dialog()
            value = dialog.colorpicker('Select color', 'ff00ff00',
                                       'os.path.join(xbmcaddon.Addon().getAddonInfo("path"), "colors.xml")')
            ..
            # colorlist example
            listitems = []
...
class DialogProgress:
    ...
    **Kodi's progress dialog class (Duh!)**
    ...
    def deallocating(self) -> None:
        ...
        It's virtual because it's a convenient place to receive messages that we're about to go be deleted but prior to
        any real tear-down.
    ...
    **Kodi's background progress dialog class**
    ...
    def create(self, heading: str, message: str = "") -> None:
        ...
        .. note::
            ...
            'heading' is used for the dialog's id. Use a unique heading. Use update() to update heading, message and
            progressbar.
...
class Action:
    ...
    **Action class.**
    ...
    This class serves in addition to identify carried out kodi_key_action_ids of Kodi and to be able to carry out
    thereby own necessary steps.
    ...
    def getId(self) -> int:
        ...
        This function returns the identification code used by the explained order, it is necessary to determine the type
        of command from kodi_key_action_ids.
...
class Window:
    ...
    **GUI window class for Add-Ons.**
    ...
    Likewise, all functions from here as well in the other window classes WindowDialog, WindowXML and WindowXMLDialog
    ...
    def onControl(self, control: Control) -> None:
        ...
        **onControl method.**
        ...
        This method will receive all click events on owned and selected controls when the control itself doesn't handle
        the message.
    ...
    def show(self) -> None:
        ...
        Shows this window by activating it, calling close() afterwards will activate the current window again.

        .. note::

            If your script ends this window will be closed to. To show it forever, make a loop at the end of your script
            or use doModal() instead.
        """
        pass
    ...
    def removeControls(self, pControls: List[Control]) -> None:
        ...
        :param : List with controls to remove
    ...
    def setProperty(self, key: str, value: str) -> None:
        ...
        .. note::
            ...
            Key is NOT case sensitive. Setting value to an empty string is equivalent to clearProperty(key). You can use
    ...
    def clearProperty(self, key: str) -> None:
        ...
        .. note::
            ...
            Key is NOT case sensitive. Equivalent to setProperty(key,'') You can use the above as keywords for arguments
            and skip certain optional arguments. Once you use a keyword, all following arguments require the keyword.
    ...
    def addControl(self, pControl: Control) -> None:
        ...
        +--------------------+---------------------------------------------------------------------------------------+
        | ControlFadeLabel   | The fade label control is used for displaying multiple pieces of text in the same     |
        |                    | space in Kodi                                                                         |
        +--------------------+---------------------------------------------------------------------------------------+
        ...
        +--------------------+---------------------------------------------------------------------------------------+
        | ControlSlider      | The slider control is used for things where a sliding bar best represents the         |
        |                    | operation at hand                                                                     |
        +--------------------+---------------------------------------------------------------------------------------+
    ...
    def addControls(self, pControls: List[Control]) -> None:
        ...
        :param : List with controls to add

        :raises TypeError: If supplied argument is not of List type, or a control is not of Control type
        :raises ReferenceError: If control is already used in another window
        :raises RuntimeError: Should not happen :-)
        """
        pass
    ...
    def getControl(self, iControlId: int) -> Control:
        ...
        controlId doesn't have to be a python control, it can be a control id from a Kodi window too (you can find id's
        in the xml files.
...
class WindowXML(Window):
    ...
    **GUI xml window class.**
    ...
    On functions defined input variable **``controlId`` (GUI control identifier)** is the on window.xml defined value
    behind type added with ``**id="..."**`` and used to identify for changes there and on callbacks.

    :param xmlFilename: string - the name of the xml file to look for.
    :param scriptPath: string - path to script. used to fallback to if the xml doesn't exist in the current skin. (eg
        xbmcaddon.Addon().getAddonInfo('path'))
    :param defaultSkin: [opt] string - name of the folder in the skins path to look in for the xml. (default='Default')
    :param defaultRes: [opt] string - default skins resolution. (1080i, 720p, ntsc16x9, ntsc, pal16x9 or pal.
        default='720p')
    :param isMedia: [opt] bool - if False, create a regular window. if True, create a mediawindow. (default=False)
    ...
    Example::
        ...
        ..
        win = xbmcgui.WindowXML('script-Lyrics-main.xml', xbmcaddon.Addon().getAddonInfo('path'), 'default', '1080i',
                                False)
        win.doModal()
        del win
        ..
    ...
    def addItem(self, item: Union[str, ListItem], position: Optional[int] = None) -> None:
        """
        Add a new item to this Window List.

        :param item: string, unicode or ListItem - item to add.
        :param position: [opt] integer - position of item to add. (NO Int = Adds to bottom,0 adds to top, 1 adds to one
            below from top,-1 adds to one above from bottom etc etc )
...
class WindowXMLDialog(WindowXML):
    ...
    **GUI xml window dialog**
    ...
    Example::
        ...
        ..
        dialog = xbmcgui.WindowXMLDialog('script-Lyrics-main.xml', xbmcaddon.Addon().getAddonInfo('path'), 'default',
                                         '1080i')
        dialog.doModal()
        del dialog
        ..
